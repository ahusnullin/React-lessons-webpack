...
export const ChatPage: FC<iChatPageProps> = ({ roomsList, addRoom, messagesList, addMessage, deleteChat }) => {
    const {chatId} = useParams();

    useEffect(() => {
        if (chatId && messagesList[chatId]?.length && messagesList[chatId].slice(-1)[0]?.author.name !== 'Bot') {
    ...
    return <>
        ...
        <Grid container spacing={2}>
            <Grid item xs={3}>
                ...
                <hr/>
                <Button variant="contained" disableElevation onClick={() => addRoom({
                    id: nanoid(),
            ...
            <Grid item xs={9}>
                ...
                >
                    <Grid>
                        <ButtonGroup variant="outlined" disabled={!chatId || chatId === "default"} aria-label="outlined primary button group" style={{ float: "right"}}>
                            <Button color="error" onClick={() => { chatId ? deleteChat(chatId) : null }}>Удалить</Button>
                        </ButtonGroup>
                    </Grid>
                    <Grid>
                        <MessageList messagesList={chatId && messagesList[chatId] ? messagesList[chatId] : []} />
                    </Grid>
                    <Grid>
